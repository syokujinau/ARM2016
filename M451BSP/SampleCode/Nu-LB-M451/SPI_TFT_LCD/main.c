...
void ILI9341_LCD_DisplayOnScreen(void)
{
    uint32_t i,j;
    uint32_t k;
    volatile uint8_t *u8ImgPtr = (uint8_t *) &u8Image[0];
    ...
    SPI_DISABLE(SPI_LCD_PORT);
    SPI_SET_DATA_WIDTH(SPI_LCD_PORT, 16);
    ...
    SPI_DISABLE(SPI_LCD_PORT);
    SPI_SET_DATA_WIDTH(SPI_LCD_PORT, 8);
    SPI_ENABLE(SPI_LCD_PORT);
